# -*- coding: utf-8 -*-
"""
Created on Tue May 27 17:41:26 2025
...
microwave_cycle = 100 * u.us //4 #50 * u.us // 4
laser_cycle = 100 * u.us // 4 #600 * u.us // 4 
...
with program() as hello_ODMR_summed:
    i = declare(int)
    n = declare(int)
    f = declare(int)
    val = declare(fixed)
    acc = declare(fixed)  # 누적 측정값
    ...
    with for_(*from_array(f, pulse_freq)):
        ...
        with while_(n < N):
            ...
            # 여러 번의 측정을 통해 SPCM 데이터 얻기
            with for_(i, 0, i < 10, i + 1):
                play(amp(1.0)*"cw", "RF3", duration=microwave_cycle)
                wait(25, "SPCM1")
                measure("readout", "SPCM1",
                        integration.full("constant", val))
                wait(25000, "SPCM1")
                assign(acc, acc + val)
    ...
    with stream_processing():
        acc_st.save_all("summed_signal")
        n_st.save_all("iterations")
...
with program() as test2:
    ...
    with while_(n < N):
        with for_(*from_array(f, pulse_freq)):
            ...
            play("laser_ON", "AOM", duration=400_000>>2) ## AOM 200us -> 400us
            play(amp(1.0)*"cw", "RF3", duration=200_000>>2) #200us
            measure("readout", "SPCM1", 
                    integration.full("constant", valA))
            # ABmeasureinterval = 100000
...
if simulate:
    ...
    if test == True:
        ...
        job = qm.execute(hello_ODMR_summed)
        
        res_handles = job.result_handles
        fetching_mode = "live"
        results = fetching_tool(job, data_list=["summed_signal", "iterations"], mode=fetching_mode)
        fig, ax = plt.subplots(figsize=(8,5))
        ...
        while results.is_processing():
            ...
            # 🔧 iteration 배열일 경우 마지막 값 추출
            iteration = iteration[-1] if isinstance(iteration, np.ndarray) else iteration
            ...
            plt.cla()
            ax.plot(pulse_freq / 1e6, summed_signal, "o-")
            ax.set_xlabel('Frequency (MHz)')
            ax.set_ylabel('Summed SPCM Signal')
