...
with program() as test2:
    ...
    with while_(n < N):
        with for_(*from_array(f, pulse_freq)):
            ...
            play("laser_ON", "AOM", duration=420_000>>2) ## AOM 200us -> 420us
...
if simulate:
    ...
    if test == True:
        ...
        res_handles = job.result_handles
        fetching_mode = "live"
        results = fetching_tool(job, data_list=["A", "iteration"], mode=fetching_mode)
        fig, ax = plt.subplots(figsize=(8,5))
        interrupt_on_close(fig, job)  # Interrupts the job when closing the figure
        
        while results.is_processing():
            A, iteration = results.fetch_all()
            progress_counter(iteration, N, start_time=results.get_start_time())
